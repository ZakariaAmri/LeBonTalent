                                ...
                                <div class="media">
                                    ...
                                    <%
                                    	...
                                    	try{
                                        	...
                                        	List<model.Demande> mesAnnonce = (List<model.Demande>) sd.getAnnonceInscrit(attribut);
                                        	List<model.Demande> mesDemandes = (List<model.Demande>) sd.getDemandeInscrit(attribut);
                                        	String affichage="";
                                        	int somme = mesDemandes.size()+mesAnnonce.size();
                                    		affichage += "<h4 class=\"media-heading\">"+attribut.getLogin()+"</h4>";
                                    		affichage += "</div></div><hr /><strong>Nombre de demande: </strong>";
                                    		if(!mesDemandes.isEmpty())
                                            	affichage +=  " <br>"+mesDemandes.size()+" demandes ";
                                    		else
                                    			affichage +=  "<br> 0 demande<hr />";

                                        		affichage += "<strong>Nombre d'annonces: </strong>";
                                        		if(!mesAnnonce.isEmpty())
                                                	affichage +=  "<br>"+mesAnnonce.size();
                                        		else
                                        			affichage +=  " <br>0 annonce<hr />";
                                        			
                                        			affichage += "<strong>Total: </strong>";
                                            		if(!mesAnnonce.isEmpty())
                                                    	affichage +=  somme+"<hr />";
                                            		else
                                            			affichage +=  "0<hr />";
